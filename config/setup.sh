...
default_configure_proxy() {
	sed '/# Setup_proxy/d' -i /developer/.bashrc
...
configure_proxy() {
	if $_interactive; then
		...
		if dialog --title 'developer config' --defaultno --yesno 'Enable UNS proxy' 5 30; then
			sed '/# Setup_proxy/d' -i /developer/.bashrc
			sudo sed '/# Setup_proxy/d' -i /etc/apt/apt.conf
			echo "source /developer/ros2_ws/src/webots_ros2_suv/docker/config/proxy.sh &>/dev/null # Setup_proxy" >>/developer/.bashrc
			source /developer/ros2_ws/src/webots_ros2_suv/docker/config/proxy.sh
...
default_first_time_ros_setup() {
	...
	cd /developer/ros2_ws && yes | rosdep --rosdistro "${ROS_DISTRO}" install -r --from-paths src --ignore-src
}

first_time_ros_setup() {
	if $_interactive; then
		if dialog --title 'developer config' --yesno 'Run first time ROS setup' 5 30; then
			default_first_time_ros_setup
			return
		fi
	fi
...
interactive_vnc() {
	...
	if dialog --title 'developer config' --yesno 'Enable VNC' 5 30; then
		VNC_CMD='/usr/bin/vnc.sh &>/dev/null'
		if dialog --title 'developer config' --yesno 'Enable GPU acceleration' 5 30; then
			VNC_CMD="VGL=-vgl ${VNC_CMD}"
		fi
		if dialog --title 'developer config' --inputbox 'Enter host X11 display server location' 8 30 "$DISPLAY" 2>/tmp/answer; then
			VNC_CMD=" DISPLAY=$(cat /tmp/answer) ${VNC_CMD}"
		fi
		if dialog --title 'developer config' --inputbox 'Enter VNC server port' 8 30 5910 2>/tmp/answer; then
			VNC_CMD=" PORT_VNC=$(cat /tmp/answer) ${VNC_CMD}"
		fi
		if dialog --title 'developer config' --inputbox 'Enter noVNC webserver port' 8 30 6810 2>/tmp/answer; then
			VNC_CMD=" PORT_WEB=$(cat /tmp/answer) ${VNC_CMD}"
		fi
		sed '/# Setup_VNC/d' -i /developer/.bashrc
		echo "$VNC_CMD # Setup_VNC" >>/developer/.bashrc
	else
		sed '/# Setup_VNC/d' -i /developer/.bashrc
	fi
}
...
finalize() {
	sed '/# Setup_shell/d' -i /developer/.bashrc
	echo 'echo "$-" | grep i -q  # Setup_shell' >>/developer/.bashrc
	clear
}
