...
4. Подключение к контейнеру через вс код(приложение code на Linux)
   В левом нижнем углу синяя иконка "Open a Remote Window", нажимаешь ему. В поисковике по центру выбирает "Attach to running container" и выбираем "/hzrob-test". Теперь мы связали VSCode и наш контейнер. Теперь нажимаем "Open Folder" и прописываем путь /developer/ros2_ws/ . Готово
